  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // Soporte prioritario para el usuario admin con clave ADMIN2026
        if (
          (inputLower === 'admin' || inputLower === 'admin@rosariodeperija.gob.ve') &&
          credentials.password.trim() === 'ADMIN2026'
        ) {
          ...
          if (adminUser) {
            return {
              id: adminUser.id,
              name: adminUser.nombres,
              email: adminUser.email,
              rol: adminUser.rol,
              cedula: adminUser.cedulaRif
            };
          }
        }
        ...
        const usuario = await prisma.usuario.findFirst({
          where: {
            OR: [
              ...
              ...(inputLower === 'admin' ? [{ email: 'admin@rosariodeperija.gob.ve' }] : []),
